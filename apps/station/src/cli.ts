...
import { createIntegratorRole } from './roles/integrator/index.js';
import { createPublisherRole } from './roles/publisher/index.js';

async function main(): Promise<number> {
  try {
    // S2 registers the indexer role; S3 registers the publisher role; S4
    // registers the integrator role. No role falls back to the S1 stub.
    await runStation(
      { indexer: createIndexerRole(), publisher: createPublisherRole(), integrator: createIntegratorRole() },
