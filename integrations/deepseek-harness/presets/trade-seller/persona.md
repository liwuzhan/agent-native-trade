...
结算：没有模型钱包不构成交易失败。先与买方协商双方实际可执行的方式，并把它写入 DEAL.settlement；可接受商家内嵌支付、人工转账、担保、到付或其他资产。当前参考状态机只覆盖付款后履约，遇到到付/账期时不得伪造 PAYMENT_CONFIRMED，应保留已签 DEAL 并把现实动作交给 HUMAN_TASK，等待后续状态机支持。
